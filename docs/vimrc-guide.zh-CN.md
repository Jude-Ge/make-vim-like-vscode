...
| 新文件 | `Ctrl-N` | 创建未命名缓冲区 |
| 打开文件 | `Ctrl-P` 或 `<Space>ff` | 输入文件名的若干字符，按顺序模糊匹配 |
| 文件树 | `Ctrl-B` 或 `<Space>e` | 打开/关闭左侧 netrw |
| 关闭编辑器 | `<Space>bd` | 关闭当前缓冲区，修改未保存时确认 |
| 上/下一个编辑器 | `[b` / `]b` | 切换缓冲区 |
| 缓冲区列表 | `<Space>bb` | 显示列表后输入编号 |
| 仅保留当前编辑器 | `<Space>bo` | 关闭其他未修改缓冲区，保留未保存内容 |
| 相对行号 | `F3` | 开关相对行号 |
| 空白字符 | `F4` | 显示/隐藏 Tab 和行尾空格 |

`Ctrl-P` 会递归建立项目文件缓存，并按输入字符的先后顺序匹配路径。例如输入 `RME` 可以匹配 `README.md`。多个结果时输入列表编号打开；超过 20 个结果时应输入更具体的查询。缓存按 Vim 当前目录保存，新增文件后执行 `:VSCodeFilesRefresh`。首次扫描大型单体仓库可能较慢，建议在项目根目录启动 Vim。
...
项目搜索完全使用 Vim 自带 `vimgrep`，默认按字面量搜索，因此输入 `[abc]`、`a+b` 或 `*.c` 不会被当作正则表达式。搜索与文件查找共用排除规则，默认跳过 `.git`、`.svn`、`node_modules`、`dist`、`build` 和常见二进制文件，同时包含根目录点文件及 `.github` 内容。它不具备 ripgrep 的速度，超大项目建议缩小 Vim 的当前目录后再搜索。
